...
class FlickrBackup:
    def __init__(self, api_key, api_secret, backup_dir):
    ...
    def setup_logging(self):
        ...
        log_file = self.backup_dir / f"flickr_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def oauth_authenticate(self):
        ...
        authorization_url = oauth.authorization_url(base_authorization_url, perms='read')
    ...
    def save_tokens(self):
        """Save OAuth tokens"""
        token_file = self.backup_dir / ".flickr_tokens.json"
        tokens = {
        ...
        with open(token_file, 'w') as f:
            json.dump(tokens, f)
        self.logger.info("Tokens saved for future use")
    ...
    def load_tokens(self):
        ...
        if token_file.exists():
            try:
                ...
                self.logger.info("Loaded saved authentication tokens")
                return True
            except Exception as e:
    ...
    def get_photoset_photos(self, photoset_id):
        ...
        while page <= pages:
            data = self.api_call('flickr.photosets.getPhotos', {
    ...
    def get_all_photos(self):
        ...
        while page <= pages:
            data = self.api_call('flickr.people.getPhotos', {
                'user_id': self.user_id,
                'extras': 'url_o,original_format,media',
                'page': page,
                'per_page': 500
    ...
    def embed_metadata(self, filepath, metadata):
        ...
        try:
            ...
            if metadata.get('title'):
                exif_dict["0th"][piexif.ImageIFD.ImageDescription] = metadata['title'].encode('utf-8')[:65535]
            
            if metadata.get('description'):
                exif_dict["0th"][piexif.ImageIFD.Copyright] = metadata['description'].encode('utf-8')[:65535]
            
            if metadata.get('tags'):
                tags_str = ', '.join(metadata['tags'])
                exif_dict["0th"][piexif.ImageIFD.XPKeywords] = tags_str.encode('utf-16le')[:65535]
            
            if metadata.get('date_taken'):
                exif_dict["Exif"][piexif.ExifIFD.DateTimeOriginal] = metadata['date_taken'].encode('utf-8')
    ...
    def download_photo(self, photo, album_folder):
        ...
        try:
            ...
            metadata = {
                ...
                'tags': [tag.get('_content', '') for tag in photo_info.get('tags', {}).get('tag', [])],
    ...
    def backup(self):
        ...
        try:
            ...
            self.logger.info("Saved tokens invalid, re-authenticating...")
            self.oauth_authenticate()
...
Examples:
  ...
  %(prog)s -k YOUR_API_KEY -s YOUR_API_SECRET -d /path/to/backup
    ...
    parser.add_argument(
        '-k', '--key',
        required=True,
        help='Flickr API Key (required)'
    )
    
    parser.add_argument(
        '-s', '--secret',
        required=True,
        help='Flickr API Secret (required)'
    )
    ...
    # Start backup
    backup = FlickrBackup(args.key, args.secret, args.dir)
